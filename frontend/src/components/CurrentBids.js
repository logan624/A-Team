...
const CurrentBids = () => {
  ...
  useEffect(() => {
    const fetchCurrentBids = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/current-bids/${username}`)
  ...
  return (
    ...
    <Container>
        <MDBTypography variant="h1" className="text-center my-4 freeline-font">Current Bids</MDBTypography>
            ...
            <MDBCard key={index} className="m-3" style={{ width: '20rem' }}>
              <MDBCardBody>
                ...
                <ul>
                  ...
                  <li>
                    <strong>Bid End Date:</strong> {item.bidEndDate}
                  </li>
                  <li>
                    <strong>Current Bid Price:</strong> {item.bidPrice}
                  </li>
                </ul>
              </MDBCardBody>
            </MDBCard>
